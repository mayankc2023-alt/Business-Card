...
function waLink(contact) {
  ...
  const text = encodeURIComponent(CATALOG_MESSAGE.replace("{name}", contact.name || "there"));
  return `https://wa.me/${digits}?text=${text}`;
}

function mailtoLink(contact) {
  if (!contact.email) return null;
  const subject = encodeURIComponent(EMAIL_SUBJECT);
  const body = encodeURIComponent(EMAIL_BODY.replace("{name}", contact.name || "there"));
...
export default function App() {
  ...
  async function runExtraction() {
    ...
    try {
      const res = await fetch("/api/extract-card", {
        ...
        body: JSON.stringify({ front: frontImg, back: backImg }),
      });
      if (!res.ok) {
  ...
  function startWaQueue() {
    const withPhone = contacts.filter((c) => waLink(c));
    if (withPhone.length === 0) return;
    setWaQueue({ ids: withPhone.map((c) => c.id), pos: 0 });
    window.open(waLink(withPhone[0]), "_blank");
  }
  ...
  function advanceWaQueue() {
    ...
    if (nextContact) window.open(waLink(nextContact), "_blank");
    setWaQueue({ ...waQueue, pos: nextPos });
  }

  function startEmailQueue() {
    const withEmail = contacts.filter((c) => mailtoLink(c));
    if (withEmail.length === 0) return;
    setEmailQueue({ ids: withEmail.map((c) => c.id), pos: 0 });
    window.location.href = mailtoLink(withEmail[0]);
  }
  ...
  function advanceEmailQueue() {
    ...
    if (nextContact) window.location.href = mailtoLink(nextContact);
      ...
      {contacts.length > 0 && (
        ...
        >
          ...
          {waQueue ? (
            ...
            <button className="ecs-btn" style={{ padding: "6px 12px", fontSize: 12 }} onClick={startWaQueue} disabled={!contacts.some((c) => waLink(c))}>
              <i className="ti ti-brand-whatsapp" style={{ marginRight: 6, verticalAlign: "-2px" }} aria-hidden="true"></i>
              Send WhatsApp to all ({contacts.filter((c) => waLink(c)).length})
            </button>
          )}
          ...
          {emailQueue ? (
            ...
            <button className="ecs-btn" style={{ padding: "6px 12px", fontSize: 12 }} onClick={startEmailQueue} disabled={!contacts.some((c) => mailtoLink(c))}>
              <i className="ti ti-mail" style={{ marginRight: 6, verticalAlign: "-2px" }} aria-hidden="true"></i>
              Email all ({contacts.filter((c) => mailtoLink(c)).length})
            </button>
          )}
        </div>
      )}
